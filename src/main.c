...
#define iqsrdy_NODE DT_ALIAS(sw0)
#define I2C_NODE DT_NODELABEL(iqs7211a)

static const struct gpio_dt_spec intr_pin = GPIO_DT_SPEC_GET(iqsrdy_NODE, gpios);
...
bool printData(void)
{
  ...
  if (
      (
          (IQS7211A_getPowerMode() != running_power_mode) || (IQS7211A_getAbsXCoordinate(FINGER_1) != running_x_output) || (IQS7211A_getAbsYCoordinate(FINGER_1) != running_y_output)))
  {
